...
function MobileMetricCard({ icon, value, label, tint }) {
  return (
    <div className="relative overflow-hidden rounded-[1.7rem] border border-[rgba(26,24,20,0.06)] bg-white/88 p-4 shadow-[0_16px_50px_rgba(26,24,20,.06)]">
      <div className="absolute inset-0 opacity-90" style={{ background: `radial-gradient(circle at 18% 12%, ${tint}, transparent 58%)` }} />
      <div className="relative z-10">
        <div className="mb-5 flex items-center justify-between">
          <div className="grid h-10 w-10 place-items-center rounded-2xl bg-white/75 text-[#C4973F] shadow-inner">
            <Icon name={icon} className="h-4 w-4" />
          </div>
          <span className="h-2 w-2 rounded-full bg-[#5B8A68]" />
        </div>

        <div className="font-display text-3xl font-black tracking-[-.05em] text-[#1A1814]">{value}</div>
        <div className="mt-1 text-[11px] font-bold leading-4 text-[#8A8278]">{label}</div>
      </div>
    </div>
  );
}
...
function MobileDashboardPreview() {
  const mobileFeed = [
    ["09:12", "DM answered", "Lip filler enquiry booked", "conversations"],
    ["08:44", "Reminder sent", "Tomorrow's consultation confirmed", "bell"],
    ["08:15", "Review requested", "Botox follow-up delivered", "star"],
    ["07:58", "Invoice chased", "Automatic payment reminder sent", "mail"],
  ];

  return (
    <div className="mt-8 rounded-[2.8rem] border border-[rgba(26,24,20,0.08)] bg-[linear-gradient(180deg,#FFFDF8_0%,#F9EDE8_100%)] p-6 shadow-[0_40px_140px_rgba(26,24,20,.1)]">
      <div className="mb-6 flex items-end justify-between">
        <div>
          <div className="text-[10px] font-extrabold uppercase tracking-[.22em] text-[#C4973F]">Mobile experience</div>
          <h3 className="mt-2 font-display text-5xl font-black tracking-[-.06em] text-[#1A1814]">The app they check more than Instagram.</h3>
        </div>
      </div>

      <div className="flex justify-center">
        <div className="relative w-[375px] overflow-hidden rounded-[3.2rem] border-[10px] border-[#1A1814] bg-[#FFFDF8] shadow-[0_45px_120px_rgba(26,24,20,.22)]">
          <div className="absolute left-1/2 top-0 z-30 h-7 w-40 -translate-x-1/2 rounded-b-[1.4rem] bg-[#1A1814]" />

          <div className="relative overflow-hidden px-5 pb-32 pt-10">
            <div className="absolute -right-10 top-20 h-40 w-40 rounded-full bg-[#F0EDF8] blur-3xl" />
            <div className="absolute -left-10 bottom-20 h-40 w-40 rounded-full bg-[#F9EDE8] blur-3xl" />

            <div className="relative z-10">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="relative inline-flex items-center justify-center">
                    <span className="absolute -top-2 left-1/2 h-5 w-[108%] -translate-x-1/2 rounded-full border-t border-[#C4973F]" />
                    <span className="font-display text-2xl font-black tracking-[-.06em] text-[#1A1814]">Lumio</span>
                  </div>
                </div>

                <div className="flex items-center gap-2 rounded-full border border-[rgba(26,24,20,0.06)] bg-white/75 px-3 py-2 text-[10px] font-bold text-[#5B8A68] shadow-sm">
                  <span className="h-2 w-2 rounded-full bg-[#5B8A68]" /> Live
                </div>
              </div>

              <div className="mt-5 flex items-center justify-between rounded-[1.8rem] border border-[rgba(26,24,20,0.06)] bg-white/75 p-4 shadow-[0_12px_40px_rgba(26,24,20,.05)] backdrop-blur-xl">
                <div>
                  <div className="font-display text-2xl font-black tracking-[-.05em] text-[#1A1814]">Glow Aesthetics</div>
                  <div className="mt-1 text-xs font-semibold text-[#8A8278]">Everything handled.</div>
                </div>
              ...
              <div className="mt-5 grid grid-cols-2 gap-3">
                <MobileMetricCard icon="lead" value="31" label="Leads" tint="#F9EDE8" />
                <MobileMetricCard icon="booking" value="19" label="Bookings" tint="#F0EDF8" />
                <MobileMetricCard icon="noshow" value="2" label="No-shows" tint="#EDF4EE" />
                <MobileMetricCard icon="money" value="£4.8k" label="Pipeline" tint="#FFF1D5" />
              </div>

              <div className="mt-6 rounded-[2rem] border border-[rgba(26,24,20,0.06)] bg-white/78 p-5 shadow-[0_20px_70px_rgba(26,24,20,.05)] backdrop-blur-xl">
                <div className="mb-5 flex items-center justify-between">
                  <div>
                    <div className="text-[10px] font-extrabold uppercase tracking-[.18em] text-[#C4973F]">Today's activity</div>
                    <div className="mt-2 font-display text-3xl font-black tracking-[-.05em] text-[#1A1814]">Live automation feed</div>
                  </div>

                  <div className="rounded-full bg-[#FFF4DD] px-3 py-2 text-[10px] font-bold uppercase tracking-[.14em] text-[#C4973F]">
                    Live
                  </div>
                </div>

                <div className="max-h-[280px] space-y-4 overflow-y-auto pr-1">
                  {mobileFeed.map(([time, title, detail, icon]) => (
                    <div key={time} className="flex gap-3 border-b border-[rgba(26,24,20,0.06)] pb-4 last:border-b-0">
                      <div className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-[#FFF4DD] text-[#C4973F]">
                        <Icon name={icon} className="h-4 w-4" />
                      </div>
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-3">
                          <div className="text-sm font-bold text-[#1A1814]">{title}</div>
                          <div className="text-[10px] font-bold text-[#8A8278]">{time}</div>
                        </div>

                        <div className="mt-1 text-xs leading-5 text-[#8A8278]">{detail}</div>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div className="absolute bottom-0 left-0 right-0 border-t border-[rgba(26,24,20,0.06)] bg-[#FFFDF8]/90 px-4 pb-7 pt-4 backdrop-blur-2xl">
            <div className="grid grid-cols-5 gap-2 rounded-[1.8rem] border border-[rgba(26,24,20,0.06)] bg-white/80 p-2 shadow-[0_16px_50px_rgba(26,24,20,.08)]">
              {[
                ["overview", "Home", false],
                ["activity", "Live", false],
                ...
                <button key={label} className={`grid place-items-center gap-1 rounded-2xl px-2 py-3 text-[10px] font-extrabold ${active ? "bg-[#C4973F] text-[#1A1814] shadow-[0_14px_30px_rgba(196,151,63,.3)]" : "text-[#8A8278]"}`}>
                  <Icon name={icon} className="h-4 w-4" />
                  {label}
                </button>
              ))}
          ...
          <button className="absolute bottom-28 right-5 z-30 flex h-16 w-16 items-center justify-center rounded-full border border-[#C4973F]/20 bg-[#1A1814] shadow-[0_0_40px_rgba(196,151,63,.35)] transition hover:scale-105">
            <div className="absolute h-12 w-12 rounded-full bg-[#E8B44B]/25 blur-xl" />
            <div className="relative h-5 w-5 rounded-full bg-[#E8B44B] shadow-[0_0_30px_rgba(232,180,75,.95)]" />

            <div className="absolute -right-1 -top-1 flex h-6 w-6 items-center justify-center rounded-full border-2 border-[#FFFDF8] bg-[#C4973F] text-[10px] font-black text-[#1A1814]">
              3
            </div>
          </button>
        </div>
...
export default function LumioDashboard() {
  return (
    <main className="relative min-h-screen overflow-hidden bg-[linear-gradient(180deg,#FFFDF8_0%,#F9EDE8_45%,#FFFDF8_100%)] px-6 py-10 text-[#1A1814] antialiased lg:px-10">
      <style>{`
      ...
      <div className="pointer-events-none absolute left-[-120px] top-[120px] h-[420px] w-[420px] rounded-full bg-[#F0EDF8] blur-3xl" />
      <div className="pointer-events-none absolute bottom-[-120px] right-[-120px] h-[420px] w-[420px] rounded-full bg-[#F9EDE8] blur-3xl" />

      <div className="relative z-10 mx-auto max-w-6xl">
        <div className="mb-10 text-center">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-[#C4973F]/15 bg-white/70 px-4 py-2 text-[10px] font-extrabold uppercase tracking-[.22em] text-[#C4973F] backdrop-blur-xl">
            <span className="h-2 w-2 rounded-full bg-[#C4973F]" />
            Lumio Mobile Experience
          </div>

          <h1 className="font-display text-6xl font-black tracking-[-.08em] text-[#1A1814]">
            The clinic app they
            <span className="italic text-[#C4973F]"> actually use.</span>
          </h1>

          <p className="mx-auto mt-5 max-w-2xl text-sm leading-8 text-[#8A8278]">
            Calm. Instant. Intelligent. Built for aesthetic clinic owners who need everything handled at a glance.
          </p>
        </div>
