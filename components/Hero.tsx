            ...
            <div className="reveal-4 flex flex-col gap-3 w-full md:w-auto">
              <div className="flex flex-col sm:flex-row sm:flex-wrap items-center gap-3 w-full sm:w-auto">
                <div className="relative group w-full sm:w-auto">
                  ...
                  <div className="pointer-events-none absolute bottom-full left-1/2 sm:left-0 -translate-x-1/2 sm:translate-x-0 mb-3 w-52 opacity-0 group-hover:opacity-100 transition-opacity duration-150 hidden md:block z-50">
                ...
                <div className="rounded-[2rem] border-[3px] border-white/10 bg-white/[0.04] p-2.5 shadow-[0_30px_80px_rgba(0,0,0,0.4)]">
                  <div className="rounded-[1.5rem] bg-[#FFFDF8] p-4" style={{ fontFamily: 'var(--font-inter, system-ui)' }}>
                    <div className="flex items-center justify-between mb-3">
                      <Logo width={70} />
                      <span className="rounded-full bg-[#EDF4EE] px-2 py-0.5 text-[9px] font-bold text-[#5B8A68]">Live</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2 mb-3">
                      {[
                        { bg: '#F9EDE8', v: '31', l: 'Leads' },
                        { bg: '#F0EDF8', v: '19', l: 'Bookings' },
                        { bg: '#EDF4EE', v: '2', l: 'No-shows' },
                        { bg: '#F2DDD8', v: '£4.8k', l: 'Pipeline' },
                      ].map(c => (
                        <div key={c.l} className="rounded-xl p-2.5" style={{ backgroundColor: c.bg }}>
                          <div className="text-lg font-black text-[#1A1814]">{c.v}</div>
                          <div className="text-[9px] font-bold text-[#8A8278]">{c.l}</div>
                        </div>
                      ))}
                    </div>
                    <div className="space-y-1.5">
                      {[
                        { t: 'Instagram DM answered', d: 'Lip filler enquiry' },
                        { t: 'Booking confirmed', d: '27 May at 11:00am' },
                      ].map(r => (
                        <div key={r.t} className="flex items-center gap-2 rounded-xl bg-[#F9EDE8]/70 px-3 py-2">
                          <div className="h-1.5 w-1.5 rounded-full bg-[#C4973F] shrink-0" />
                          <div>
                            <p className="text-[9px] font-bold text-[#1A1814]">{r.t}</p>
                            <p className="text-[8px] text-[#8A8278]">{r.d}</p>
                          </div>
                        </div>
                      ))}
                    </div>
